...
print([movie['name'] for movie in highly_rated_movies])
...
average_imdb_score = compute_average_imdb_score(movies)

print(average_imdb_score)

# 5.
def com_av_by_cat(movies, category):
    fm = [movie for movie in movies if movie['category'] == category]
    total_score = sum(movie['imdb'] for movie in fm)
    avsc = total_score / len(fm) if fm else 0
    return avsc
